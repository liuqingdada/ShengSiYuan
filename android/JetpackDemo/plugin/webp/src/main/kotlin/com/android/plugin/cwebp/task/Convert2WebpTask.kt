...
import com.android.plugin.cwebp.Convert2WebpExtension
import com.android.plugin.cwebp.util.ImageUtil
...
//                println("--- ${file.absolutePath}")
        ...
        if (imageFileList.size < coreNum) {
            ...
            for (f in results) {
                try {
                    ...
                    println("CwebpPlugin Convert2WebpTask#dispatchOptimizeTask() execute wrong.")
                }
            }
        }
    }
    ...
    private fun calcNewSize(path: String) {
        if (File(path).exists()) {
            ...
            if (File(webpPath).exists()) {
                ...
                println("CwebpPlugin Convert2Webp Task was wrong.")
            }
        }
    }
    ...
    private fun checkCwebpTools() {
        ...
        if (!WebpToolBean.getToolsDir().exists()) {
            throw GradleException("CwebpPlugin 'convert2Webp' task need cwebp tool.")
        }
    }
